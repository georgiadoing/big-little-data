...
def get_options():
    ...
    parser.add_argument('prefix', action='store',
                        help='Prefix for created files', default='temp_')
...
if __name__ == "__main__":
    ...
    sns.despine(left=True,
                bottom=True)
    plt.savefig('%spangenome_frequency.%s'%(options.prefix,options.format), dpi=300)
    plt.clf()
    ...
    with sns.axes_style('whitegrid'):
        ...
        plt.savefig('%spangenome_matrix.%s'%(options.prefix,options.format), dpi=300)
        plt.clf()
    ...
    a=plt.pie([core, softcore, shell, cloud],
          ...
          autopct=my_autopct)
    plt.savefig('%spangenome_pie.%s'%(options.prefix,options.format), dpi=300)
    plt.clf()
