...
ec_studies <- sapply(colnames(ec_counts)[-c(1,2,3)], function(x) 
  ec_run_table$SRA.Study[ec_run_table$Experiment == str_extract(x, '[ES]RX[0-9]*')][1])
ec_studies[is.na(ec_studies)] <- "NA"

ec_strains <- sapply(colnames(ec_counts)[-c(1,2,3)], function(x) 
  ec_run_table$strain_put[ec_run_table$Experiment == str_extract(x, '[ES]RX[0-9]*')][1])
ec_strains[is.na(ec_strains)] <- "NA"
```
...
ec_gene_pa <- read.csv('ecoli_lim_outv3/gene_presence_absence.csv', stringsAsFactors = F)
ec_gene_anns <- read.csv('ecoli_pan_genome/ecoli_pan_genome.tsv', sep = '\t', stringsAsFactors = F)

gpa_anns <- read.csv('ecoli_pan_genomev3_gene_presence_absence_ann.csv', stringsAsFactors = F)

sum(gpa_anns$first_name_comp %in% ec_counts$X)
...
```{r}
sum(!(is.na(gpa_anns$MG1655)))

mg1655_only <- str_detect(ec_counts$X, "11997cc26382b2c286cd502685a104a5")

mg1655_select_all <- gpa_anns$first_name_comp[!(is.na(gpa_anns$MG1655))]
```

```{r, warning=F, message=F,, fig.width=12, fig.height=6}

rownames(ec_counts) <- ec_counts$X
length(mg1655_select_all)
sum(mg1655_select_all %in% rownames(ec_counts))

mg1655_select <- mg1655_select_all[mg1655_select_all   %in% rownames(ec_counts)]

hist((rowSums(ec_counts[mg1655_select,-c(1,2,3)] == 0, na.rm=T) ))
```
...
```{r, warning=F, message=F,, fig.width=12, fig.height=6}
ec_log_counts <- log(ec_counts[mg1655_select,-c(1,2,3)] + 1)

sum(rowSums(ec_counts[mg1655_select,-c(1,2,3)] == 0, na.rm=T) < 19000 )
...
hist(log(rowSums(ec_counts[mg1655_select,-c(1,2,3)])))
```
...
ec_counts_t <- ec_counts[mg1655_select,-c(1,2,3)]
...
ann_colors <- rep(brewer.pal(6,'Greys'),214)[1:length(unique(ec_studies))]
names(ann_colors) <- unique(ec_studies)
st_colors <- rep(brewer.pal(8,'Dark2'),6)[1:length(unique(ec_strains))]
names(st_colors) <- unique(ec_strains)

ha <- HeatmapAnnotation(study = ec_studies[c(1:1000)], 
                        strain = ec_strains[c(1:1000)],
                        show_legend = c(F,T), 
                        annotation_name_side = "left",
                        col = list( study = ann_colors, 
                                    strain = st_colors))

h <- Heatmap(ec_log_counts[,c(1:1000)], col = col_fun, 
             top_annotation = ha,
             height = unit(8,"cm"),
             width = unit(24,"cm"),
             cluster_columns = F,
...
```{r}
ec_counts_all <- ec_counts
ec_counts <- ec_counts[mg1655_select,]
```
...
ha <- HeatmapAnnotation(study = ec_studies[c(1:1000)], 
                        strain = ec_strains[c(1:1000)],
                        filter = ec_counts_filtered[c(1:1000)],
                        show_legend = c(F,T,T), 
                        annotation_name_side = "left",
                        col = list( study = ann_colors,
                                    strain = st_colors,
...
ha <- HeatmapAnnotation(study = ec_studies[ec_counts_filtered][c(1:1000)], 
                        strain = ec_strains[ec_counts_filtered][c(1:1000)],
...
ann_colors <- rep(brewer.pal(8,'Dark2'),3)[1:length(unique(ec_studies))]
ann_colors <- colorRampPalette(brewer.pal(6, "Greys"))(length(unique(ec_studies)))
ann_colors <- rep(brewer.pal(6,'Greys'),500)[1:length(unique(ec_studies))]
names(ann_colors) <- unique(ec_studies)
st_colors <- rep(brewer.pal(8,'Dark2'),5)[1:length(unique(ec_strains))]
names(st_colors) <- unique(ec_strains)

ha <- HeatmapAnnotation(study = ec_studies[ec_counts_filtered][ec_strains[ec_counts_filtered] == "MG1655"], 
                        strain = ec_strains[ec_counts_filtered][ec_strains[ec_counts_filtered] == "MG1655"],
                        #filter = ec_counts_filtered,
                        show_legend = c(F,T,T), 
                        annotation_name_side = "left",
                        col = list( study = ann_colors,
...
h <- Heatmap(ec_log_counts[,ec_strains[ec_counts_filtered] == "MG1655"], col = col_fun, 
             top_annotation = ha,
             height = unit(8,"cm"),
             width = unit(24,"cm"),
             cluster_columns = F,
...
ec_counts_norm  <- MRnorm(ec_counts[,c(F,F,F,ec_counts_filtered)]+1)
...
ec_counts_norm_t <- ec_counts_norm
...
h <- Heatmap(ec_log_counts_norm_t[,c(1:1000)], col = col_fun, 
             top_annotation = ha,
             height = unit(8,"cm"),
             width = unit(24,"cm"),
             cluster_columns = F,
...
#write.csv(ec_log_counts_norm_01, 'compendia/ecoli_MG1655_genome_log_counts_norm_01_nonames.csv', row.names = F)
#write.csv(ec_log_counts_norm_01, 'compendia/ecoli_MG1655_genome_log_counts_norm_01.csv', row.names = T)
...
```{r, fig.width=4, fig.height=4}
ec_counts_norm  <- MRnorm(ec_counts[,c(F,F,F,(ec_counts_filtered & ec_strains == "MG1655"))]+1)
rownames(ec_counts_norm) <- ec_counts$X
hist(log(rowSums(ec_counts_norm)))
```
...
```{r, warning=F, message=F,, fig.width=12, fig.height=6}
ec_log_counts <- log(ec_counts_norm + 1)
...
#write.csv(ec_log_counts_norm_01, 'compendia/ecoli_MG1655_samps_only_log_counts_norm_01_nonames.csv', row.names = F)
#write.csv(ec_log_counts_norm_01, 'compendia/ecoli_MG1655_samps_only_log_counts_norm_01.csv', row.names = T)
