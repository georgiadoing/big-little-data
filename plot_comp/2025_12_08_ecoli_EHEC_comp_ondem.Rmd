---
title: "Ecoli Pangenome Compendium - full filt - ondemand kernel"
...
```{r}
#osfc <- osf_retrieve_node("https://osf.io/j4x63")
#osf_files <- osf_ls_files(osfc)
#osf_download(osf_files[])
```

```{r}

ec_counts <- read.csv('../compendia/num_reads_ecoli_pan_genome_part2_comp_ref_k15v3.csv', stringsAsFactors = F)
colnames(ec_counts)[c(4:ncol(ec_counts))] <- sapply(colnames(ec_counts)[c(4:ncol(ec_counts))],
                                                    function(x) str_extract(x, "[A-Z0-9a-z]*"))
...
sum(!is.na(gpa$MG1655))

mg1655_genes <- gpa$first_name_comp[!is.na(gpa$MG1655) ]
ehec_genes <- gpa$first_name_comp[!is.na(gpa$O157_H7_EHEC) ]
bl21_genes <- gpa$first_name_comp[!is.na(gpa$BL21_DE3) ]
upec_genes <- gpa$first_name_comp[!is.na(gpa$UPEC_536) ]


plus_genes <- unique(c(mg1655_genes,ehec_genes)) # ,bl21_genes
sum(ec_counts$X %in% plus_genes)

plus_genes <- unique(c(mg1655_genes)) # ,bl21_genes
sum(ec_counts$X %in% plus_genes)

plus_genes <- unique(c(ehec_genes)) # ,bl21_genes
sum(ec_counts$X %in% plus_genes)



ec_counts <- ec_counts[ec_counts$X %in% plus_genes,]
```
...
```{r}
ec_run_table_xtra <- read_excel('../compendia/Ecoli_GEO_strains_working_v1.xlsx', sheet = 1 )
...
ec_run_table_acc <- read_excel('../compendia/Ecoli_GEO_strains_working_v1.xlsx', sheet = 2 )
ec_run_table_acc <- ec_run_table_acc[-1,]


ec_run_table <- read.csv('../compendia/SraRunTable_Ec_04_29_25.csv', stringsAsFactors = F)

ec_counts <- ec_counts[,colnames(ec_counts) %in% c("X",ec_run_table$Experiment)]
...
```{r}
ec_studies <- sapply(colnames(ec_counts)[-c(1,2,3)], function(x) 
  ec_run_table$SRA.Study[ec_run_table$Experiment == x][1])

ec_strains <- sapply(colnames(ec_counts)[-c(1,2,3)], function(x) 
...
hist(log10(rowSums(ec_counts[,-c(1,2,3)])))

ec_log_counts <- log10(ec_counts[,-c(1,2,3)] + 1)

hist(log10(rowSums(ec_log_counts)))
```


```{r}
sum(rowSums(ec_counts[,-c(1,2,3)]) < 2)
```


```{r, warning=F, message=F,, fig.width=12, fig.height=6, eval=T}
col_fun = colorRamp2(c(min(ec_log_counts), median(data.matrix(ec_log_counts)), max(ec_log_counts)), c("grey", "#E7D4E8", "#762A83"))

rows_sel <- sample(c(1:nrow(ec_counts)), 500)

study_select <- sample(unique(ec_studies),100)
cols_sel <- ec_studies %in% study_select
ec_log_counts_temp <- ec_log_counts[rows_sel, cols_sel]


ann_colors <- rep(brewer.pal(8,'Dark2'),161)[1:length(unique(ec_studies))]

ann_colors <- rep(brewer.pal(6,'Greys'),215)[1:length(unique(ec_studies))]
names(ann_colors) <- unique(ec_studies)

st_colors <- rep(brewer.pal(8,'Dark2'),7)[1:length(unique(ec_strains))]
names(st_colors) <- unique(ec_strains)

ha <- HeatmapAnnotation(study = ec_studies[cols_sel], 
                        strain = ec_strains[cols_sel],
                        show_legend = c(F,T), 
                        col = list( study = ann_colors, 
                                    strain = st_colors))

h <- Heatmap(ec_log_counts_temp, col = col_fun, 
             top_annotation = ha,
             height = unit(2,"in"),
             width = unit(3,"in"),
             cluster_columns = F,
...
#rows_sel <- sample(c(1:nrow(ec_counts)), 100)
#cols_sel <- sample(c(1:ncol(ec_counts)), 100)
#ec_log_counts_temp <- ec_log_counts[rows_sel, cols_sel]
...
h <- Heatmap(ec_log_counts[rows_sel, cols_sel], col = col_fun, 
             top_annotation = ha,
             height = unit(8,"cm"),
             width = unit(24,"cm"),
             cluster_columns = F,
...
## Se initital heatmap - low count genes trimmed

```{r, warning=F, message=F,, fig.width=12, fig.height=6}

trim_sel <- rownames(ec_counts)[rowSums(ec_counts[,-c(1,2,3)]) > 10000]
ec_counts_t <- ec_counts[trim_sel,-c(1,2,3)] #exp(15)


ec_log_counts_t <- log10(ec_counts_t + 1)

hist(rowSums(ec_log_counts_t))

hist(log10(rowSums(ec_log_counts_t)))
...
h <- Heatmap(ec_log_counts[intersect(rows_sel, trim_sel), cols_sel], col = col_fun, 
             top_annotation = ha,
             height = unit(8,"cm"),
             width = unit(24,"cm"),
             cluster_columns = F,
...
## Subset of more highly expressed genes
...
rows_sel_high <- sample(trim_sel, 100)



h <- Heatmap(ec_log_counts[rows_sel_high, cols_sel], col = col_fun, 
             top_annotation = ha,
             height = unit(8,"cm"),
             width = unit(24,"cm"),
             cluster_columns = F,
...
col_fun2 = colorRamp2(c(min(ec_log_counts[rows_sel_high, cols_sel]),
                       #median(data.matrix(ec_log_counts[rows_sel_high, cols_sel])),
                       quantile(data.matrix(ec_log_counts[rows_sel_high, cols_sel]), probs = 0.9),
                       max(ec_log_counts[rows_sel_high, cols_sel])),
                     c("grey", "#E7D4E8", "#762A83"))



h <- Heatmap(ec_log_counts[rows_sel_high, cols_sel], col = col_fun2, 
             top_annotation = ha,
             height = unit(8,"cm"),
             width = unit(24,"cm"),
             cluster_columns = F,
...
ec_hk_gene_list <-  read_excel('../compendia/Ecoli_GEO_strains_working_v1.xlsx', sheet = 3 )
ec_gene_pa <- read.csv('ecoli_lim_outv3_gene_presence_absence.csv', stringsAsFactors = F)
ec_gene_anns <- read.csv('ecoli_pan_genome.tsv', sep = '\t', stringsAsFactors = F)
...
ec_hk_genes_names <- ec_hk_genes$Gene[ec_hk_genes$No..isolates == 19]
ec_hk_genes_loci <- ec_hk_genes$GCF_000005845.2[ec_hk_genes$No..isolates == 19]
...
hk_probs <- quantile(apply(ec_hk_counts[,-c(1,2,3)],2,FUN = 
                            function(x) log(max(median(x),1))), 
                    probs = c(.2,.99))

#pdf('plots/se16_comp_hk_counts.pdf', height=4, width=4)
plot(hist(apply(ec_hk_counts[,-c(1,2,3)],2,FUN = function(x) log(max(median(x),1))), 
          breaks = 75, plot=F),
...
hk_rows <- rownames(ec_counts)[ec_counts$X %in% ec_hk_loci]

row_ha = rowAnnotation("set" = c(rep("random",length(rows_sel_high)),
                                 rep("HK", length(hk_rows))),
                       col = list(set = c("random" = "white", "HK" = "red")))


h <- Heatmap(ec_log_counts[c(rows_sel_high, hk_rows), cols_sel], col = col_fun, 
             top_annotation = ha,
...
```{r, warning=F, message=F,, fig.width=4, fig.height=4}
zc_probs <- quantile(apply(ec_counts[,-c(1,2,3)],2,FUN = function(x) log(max(sum(x==0),1))), 
                    probs = c(.05,.95))
#pdf('plots/se16_comp_zero_counts.pdf', height=4, width=4)
plot(hist(apply(ec_counts[,-c(1,2,3)],2,FUN = function(x) log(max(sum(x==0),1))),
          breaks = 50, plot=F),
...
```{r}
hk_medians <- apply(ec_hk_counts[,-c(1,2,3)],2,FUN = 
                            function(x) log(max(median(x),1)))
zero_counts <- apply(ec_counts[,-c(1,2,3)],2,FUN = 
...
write.csv(filt_run_table, "SraRunTable_Ec_04_29_25_FILTEREDv3_MG1655plus.csv")
...
write.csv(ec_run_table_out, "SraRunTable_Ec_04_29_25_working_v3_part2_MG1655plus.csv")
```

```{r}
sum(rowSums(ec_counts[,c(F,F,F,!ec_counts_filtered)]) > 0)
```
...
ha <- HeatmapAnnotation(study = ec_studies[cols_sel], 
                        strain = ec_strains[cols_sel],
                        filter = ec_counts_filtered[cols_sel],
                        show_legend = c(F,T,T), 
                        annotation_name_side = "left",
                        col = list( study = ann_colors,
                                    strain = st_colors,
...
```{r, warning=F, message=F,, fig.width=12, fig.height=6, eval=T}
filt_sel <- c(1:ncol(ec_log_counts))[ec_counts_filtered]
cols_sel <- c(1:ncol(ec_log_counts))[cols_sel]
ha <- HeatmapAnnotation(study = ec_studies[intersect(filt_sel, cols_sel)] , 
                        strain = ec_strains[intersect(filt_sel, cols_sel)],
...
h <- Heatmap(ec_log_counts[c(rows_sel_high, hk_rows),intersect(filt_sel, cols_sel)], 
             col = col_fun, 
             top_annotation = ha,
             left_annotation = row_ha,
             height = unit(8,"cm"),
...
```{r}
temp <- ec_counts[,c(F,F,ec_counts_filtered)]
tempm <- data.matrix(ceiling(temp))

hist(log(rowSums(tempm)))

```


```{r, fig.width=4, fig.height=4}
ec_counts_norm  <- MRnorm(ec_counts[rowSums(tempm) > 0,c(F,F,ec_counts_filtered)] + 1)
rownames(ec_counts_norm) <- ec_counts$X[rowSums(tempm) > 0]
hist(log(rowSums(ec_counts_norm)))

ec_counts_norm_log <- log10(ec_counts_norm + 1)

cols_sel_rx <- colnames(ec_counts)[intersect(filt_sel, cols_sel)]
cols_sel_rx <- cols_sel_rx[cols_sel_rx %in% colnames(ec_counts_norm_log)]
```
...
ha <- HeatmapAnnotation(study = ec_studies[cols_sel_rx], 
                        strain = ec_strains[cols_sel_rx],
                        #filter = ec_counts_filtered,
                        show_legend = c(F,T,T), 
                        annotation_name_side = "left",
                        col = list( study = ann_colors,
...
h <- Heatmap(ec_counts_norm_log[as.numeric(c(rows_sel, hk_rows)),cols_sel_rx], 
             col = col_fun, 
             top_annotation = ha,
             #left_annotation = row_ha,
             height = unit(8,"cm"),
...
```{r, fig.width=4, fig.height=4}
ec_counts_norm_t  <- MRnorm(ec_counts_t[,c(ec_counts_filtered)] + 1)
rownames(ec_counts_norm_t) <- ec_counts_t$X
hist(log(rowSums(ec_counts_norm_t)))

ec_counts_norm_t_log <- log10(ec_counts_norm_t + 1)
...
write.csv(ec_counts_norm, 'compendia/ecoli_MG1655plus_genome_clean_norm_part2.csv', row.names = F)
write.csv(ec_log_counts_norm, 'compendia/ecoli_MG165plus_genome_clean_norm_log_part2.csv', row.names = F)
           #write.csv(ec_counts_norm_t, 'compendia/ecoli_MG165_genome_trimmed_clean_norm_part2.csv', row.names = F)
#write.csv(ec_log_counts_norm_t, 'compendia/ecoli_MG165_genome_trimmed_clean_norm_log_part2.csv', row.names = F)
...
write.csv(ec_log_counts_norm_01, 'compendia/ecoli_MG165plus_genome_part2_log_counts_norm_01_nonames.csv', row.names = F)
write.csv(ec_log_counts_norm_01, 'compendia/ecoli_MG165plus_genome_part2_log_counts_norm_01.csv', row.names = T)

#write.csv(ec_log_counts_norm_t_01, 'compendia/ecoli_MG165_genome_part2_trimmed_log_counts_norm_01_nonames.csv', row.names = F)
#write.csv(ec_log_counts_norm_t_01, 'compendia/ecoli_MG165_genome_part2_trimmed_log_counts_norm_01.csv', row.names = T)
```
