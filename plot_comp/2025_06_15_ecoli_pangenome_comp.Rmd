...
hist(log(rowSums(ec_counts[,-c(1,2,3)])))

ec_log_counts <- log(ec_counts[,-c(1,2,3)] + 1)

hist(log(rowSums(ec_log_counts)))
```


```{r}
sum(rowSums(ec_counts[,-c(1,2,3)]) < 150)
```
...
ec_counts_t <- ec_counts[rowSums(ec_counts[,-c(1,2,3)]) > exp(15),-c(1,2,3)]

ec_log_counts_t <- log(ec_counts_t + 1)

hist(log(rowSums(ec_log_counts_t)))
```
...
write.csv(ec_run_table_out, "SraRunTable_Ec_04_29_25_working_v3.csv")
```

```{r}
sum(rowSums(ec_counts[,c(F,F,F,!ec_counts_filtered)]) > 20000)
```
